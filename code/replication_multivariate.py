...
SIZE=2**11; d=40;error='mix';model='linear';
replication=100
...
for r in range(replication):
    ...
    data = gen_multi(model=model,d=d,size=SIZE,error=error,df=2,sigma=1); # Training data
    data_test = gen_multi(model=model,d=d,size=10000,error=error,df=2,sigma=1); # Testing data
    x=data[:][0];y=data[:][1];
    x_test=data_test[:][0];y_test=data_test[:][1];
    fx_test=y_test-data_test[:][2].data.numpy();
    for (k,method) in enumerate(methods):
        if (method!='KQR') and (method!='MARS'):
            net=DNN(width_vec=[d,256,256,256,256,256,1]);
            net=DRR(net=net,data=data,loss=method,tunepara=None,epoch=1500,batch_size=None,stopping='early',min_epoch=400,patience=100,lr=1e-3)
            y_drr=net(data_test[:][0]).detach();
            del net
            gc.collect();
