...
SIZE=2**9; model='dopller';error='normal';replication=100
...
for r in range(replication):
    ...
    for (l,width_vec) in enumerate(width_vec_list):
            net=DNN(width_vec)
            net=DRR(net=net,data=data,loss='ls',tunepara=None,epoch=1500,batch_size=None,stopping='early',min_epoch=400,patience=100,lr=1e-3)
            y_drr=net(data_test[:][0]);
            del net
            gc.collect();
            stat_ls[r,l]=(loss_ls(y_test,y_drr)-loss_ls(y_test,fx_test)).detach();
            del y_drr
            gc.collect();
